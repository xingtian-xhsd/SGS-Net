...
class Local_block(nn.Module):
    ...
    def __init__(self, dim, drop_rate=0., mlp_ratio=3):
        super().__init__()
...
class ConvolutionalGLU(nn.Module):
    def __init__(self, in_features, act_layer=nn.GELU, drop=0.):
        ...
        self.fc1 = nn.Conv2d(in_features, in_features * 2, 1)
        self.dwconv = DWConv(in_features * 2)
        self.act = act_layer()
        self.fc2 = nn.Conv2d(in_features * 2, in_features, 1)
    ...
    def forward(self, x):
        ...
        x = self.fc1(x)
        shortcut = self.fc1(shortcut)

        x = self.act(self.dwconv(x)) * shortcut
...
class HFF_block(nn.Module):
    def __init__(self, ch_1, ch_2, r_2, ch_int, ch_out, drop_rate=0.):
        ...
        self.glu = ConvolutionalGLU(ch_1 + ch_2 + ch_int)
...
class Global_block(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, mlp_ratio=3, ):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
